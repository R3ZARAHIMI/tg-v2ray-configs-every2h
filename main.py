    ...
    async def find_raw_configs_from_chat(self, chat_id: int, limit: int, retries: int = 3):
        ...
        try:
            ...
            async for message in self.client.get_chat_history(chat_id, limit=limit):
                ...
                if main_text:
                    texts_to_scan.append(main_text)
                
                # 2️⃣ استخراج متن از Entity های خاص (QUOTE به جای BLOCKQUOTE)
                if message.entities:
                    for entity in message.entities:
                        if entity.type in [enums.MessageEntityType.CODE, enums.MessageEntityType.PRE, enums.MessageEntityType.QUOTE]:
                            raw_segment = main_text[entity.offset : entity.offset + entity.length]
                            texts_to_scan.append(raw_segment)
                            texts_to_scan.append(raw_segment.replace('\n', '').replace(' ', ''))
                
                # 3️⃣ چک کردن پیام‌های Reply شده
                if message.reply_to_message:
                    replied_text = message.reply_to_message.text or message.reply_to_message.caption or ""
                    if replied_text:
                        texts_to_scan.append(replied_text)
                        if message.reply_to_message.entities:
                            for ent in message.reply_to_message.entities:
                                if ent.type in [enums.MessageEntityType.CODE, enums.MessageEntityType.PRE, enums.MessageEntityType.QUOTE]:
                                    raw_seg = replied_text[ent.offset : ent.offset + ent.length]
                                    texts_to_scan.append(raw_seg)
                                    texts_to_scan.append(raw_seg.replace('\n', '').replace(' ', ''))

                # 4️⃣ استخراج Base64
                for b64_str in BASE64_PATTERN.findall(main_text):
                    try:
