...
# ریجکس بهبود یافته برای جلوگیری از گرفتن کاراکترهای مزاحم انتهای لینک
V2RAY_PATTERNS = [
...
class V2RayExtractor:
    ...
    def parse_vless(self, vless_url: str) -> Optional[Dict[str, Any]]:
        # اصلاح علامت + در کوئری استرینگ قبل از پارس کردن
        query_part = vless_url.split('?', 1)[1].split('#', 1)[0] if '?' in vless_url else ""
        q = {k: v[0] for k, v in parse_qs(query_part).items()}
        
        # برگرداندن علامت + که توسط parse_qs به فاصله تبدیل شده بود
        ...
        if q.get('security') == 'reality' and pbk:
            ...
            if sid: reality_opts['short-id'] = sid # فقط اگر وجود داشت اضافه شود
        ...
        return {
            ...
            'ws-opts': {'path': q.get('path', '/'), 'headers': {'Host': q.get('host', '')}} if q.get('type') == 'ws' else None,
    ...
    def parse_trojan(self, trojan_url: str) -> Optional[Dict[str, Any]]:
        ...
        return {'name': unquote(p.fragment or ''), 'type': 'trojan', 'server': p.hostname, 'port': p.port or 443, 'password': p.username, 'udp': True, 'sni': q.get('sni'), 'network': q.get('type', 'tcp'), 'ws-opts': {'path': q.get('path', '/'), 'headers': {'Host': q.get('host', '')}} if q.get('type')=='ws' else None, 'client-fingerprint': q.get('fp')}
    ...
    def extract_configs_from_text(self, text: str) -> Set[str]:
        ...
        for url in found:
            url = url.strip()
            # حذف علامت # و نام کانفیگ برای پارس کردن درست پارامترها
