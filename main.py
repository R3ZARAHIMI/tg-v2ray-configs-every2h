...
class V2RayExtractor:
    ...
    def parse_vmess(self, vmess_url):
        ...
        return {
            'name': self._generate_unique_name(config.get('ps', ''), "vmess"), 'type': 'vmess', 'server': config.get('add'), 
            'port': int(config.get('port', 443)), 'uuid': config.get('id'), 'alterId': int(config.get('aid', 0)),
            'cipher': config.get('scy', 'auto'), 'tls': config.get('tls') == 'tls', 'network': config.get('net', 'tcp'), 'udp': True,
            'ws-opts': {'path': config.get('path', '/'), 'headers': {'Host': config.get('host', '')}} if config.get('net') == 'ws' else None
        }

    def parse_vless(self, vless_url):
        parsed = urlparse(vless_url); query = parse_qs(parsed.query)
        return {
            'name': self._generate_unique_name(unquote(parsed.fragment), "vless"), 'type': 'vless', 'server': parsed.hostname, 
            'port': parsed.port or 443, 'uuid': parsed.username, 'udp': True, 'tls': query.get('security', [''])[0] == 'tls',
    ...
    def parse_trojan(self, trojan_url):
        parsed = urlparse(trojan_url); query = parse_qs(parsed.query)
        return {'name': self._generate_unique_name(unquote(parsed.fragment), "trojan"), 'type': 'trojan', 'server': parsed.hostname, 
                'port': parsed.port or 443, 'password': parsed.username, 'udp': True, 'sni': query.get('peer', [None])[0] or query.get('sni', [None])[0]}

    def parse_shadowsocks(self, ss_url):
        parsed = urlparse(ss_url); user_info = '';
        if '@' in parsed.netloc:
    ...
    async def find_raw_configs_from_chat(self, chat_id, limit):
        ...
        except FloodWait as e: print(f"⏳ Waiting {e.value}s for {chat_id}..."); await asyncio.sleep(e.value); await self.find_raw_configs_from_chat(chat_id, limit)
...
async def main():
    ...
    async with extractor.client:
        ...
        tasks.extend(extractor.find_raw_configs_from_chat(gr, GROUP_SEARCH_LIMIT) for gr in GROUPS)
